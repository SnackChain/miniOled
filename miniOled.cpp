...
void OLED::sendCommand(byte command){
	SoftWire.beginTransmission(OLED_ADDRESS); // begin transmitting
	SoftWire.write(OLED_COMMAND_MODE);//data mode
	SoftWire.write(command);
	SoftWire.endTransmission();    // stop transmitting
}
...
void OLED::sendData(byte data){
	...
	SoftWire.beginTransmission(OLED_ADDRESS); // begin transmitting
	SoftWire.write(OLED_DATA_MODE);//data mode
	SoftWire.write(data);
	SoftWire.endTransmission();    // stop transmitting
}
...
void OLED::init(int sda, int scl){  // customise sda.scl ignored for regular Wire
    ...
    SoftWire.setRxBuffer(ibuffer, 60);  // common buffer is OK, but rx buffer not used anyway
    SoftWire.setTxBuffer(ibuffer, 60);
    if(sda>=0) // SDA SCL optional parameters. Otherwise defaults apply 
    {
		SoftWire.setSda(sda);   // non-default pins
		SoftWire.setScl(scl);
    }
